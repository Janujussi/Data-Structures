...
//		Precondition: head_ptr is the head pointer of the linked list which
//			contains repeated values.
//		Postcondition: the linked list does not contain any repetitions.
//
// 	void sort_list(node* &head_ptr)
//		Precondition: head_ptr is the head pointer of the linked list.
//		Postcondition: the linked list is sorted from smallest to largest.
//
//	void split_list(node* &head_ptr, node* &second_ptr, double splitValue)
//		Precondition: head_ptr is the head ponter of a linked list,
//			second_ptr is the head pointer of another linked list, splitValue is
//			the value for the linked list to be split by.
//		Postcondition: head_ptr contains the elements before the value splitValue,
//			second_ptr contains the elements including splitValue and after.
//
...
namespace main_savitch_5
{
	...
	std::size_t list_length(node *head_ptr);
	void list_head_insert(node *&head_ptr, const node::value_type &entry);
	void list_insert(node *previous_ptr, const node::value_type &entry);
	node *list_search(node *head_ptr, const node::value_type &target);
	const node *list_search(const node *head_ptr, const node::value_type &target);
